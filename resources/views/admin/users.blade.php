                    ...
                    <table id="usersTable" style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: #f5f5f5; border-bottom: 2px solid #ddd;">
                                ...
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #333;">Email Verified</th>
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr style="border-bottom: 1px solid #ddd;">
                                    ...
                                    <td style="padding: 12px; color: #666;">{{ $user->email_verified_at ? $user->email_verified_at->format('M d, Y H:i') : '—' }}</td>
                                    <td style="padding: 12px; color: #666;">{{ $user->phone ?? '—' }}</td>
                                    <td style="padding: 12px; color: #333;">{{ ucfirst($user->role) }}</td>
                                    <td style="padding: 12px;">{{ $user->is_active ? 'Active' : 'Inactive' }}</td>
