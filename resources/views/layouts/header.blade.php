    ...
    <div style="display: flex; align-items: center; gap: 30px;">
        ...
        <nav style="display: flex; gap: 12px;">
            @auth
                ...
                @else
                    <a href="#products" style="color: #fff; text-decoration: none; font-size: 13px; font-weight: normal; margin-left: 12px;">PRODUCTS</a>
                @endif
            @endauth
        </nav>
    </div>
        ...
        @auth
            ...
            <div id="profileDropdown" style="display: none; position: absolute; top: 40px; right: 0; background: #fff; border: 1px solid #ddd; border-radius: 4px; overflow: hidden; z-index: 100; min-width: 150px;">
                <form method="POST" action="{{ route('logout') }}" style="margin: 0;">
                    @csrf
                    <button type="submit" style="width: 100%; text-align: left; padding: 12px 16px; border: none; background: none; color: #333; cursor: pointer; font-size: 13px; font-weight: normal;">Logout</button>
                </form>
            </div>
        @endauth
    </div>
